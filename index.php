...
{  
    ...
    echo '<header class="pat-res-header"><h2 class="pat-res-section-heading">' . ucwords(str_replace('-',' ',$folder_path['basename'])) . '<a href="#home" class="pat-res-btt">Back to top</a></h2>';
    $info_file = $folder . '/info.txt';
    if (file_exists($info_file)) {
    	echo '<div class="pat-res-info pat-res">';
			include($info_file);
			echo '</div></header>';
    }
    else {
    ...
    foreach(glob($folder . '/*') as $file)  
		{
			...
			if (is_dir($file) === true) {
				...
				echo '<header class="pat-res-sub-header pat-res"><h3 class="pat-res-pattern-heading">' . ucwords(str_replace('-',' ',$file_path['basename'])) . '</h3>';
...
function print_html($folder, $file, $file_path, $folder_name) {
	...
	$max_width_string = find_string($file_contents, '<!--MAXWIDTH!', '/MAXWIDTH-->');
		if (!$folder_name) {
		echo '<header class="pat-res-sub-header pat-res"><h3 class="pat-res-pattern-heading">' . ucwords(str_replace('-',' ',$file_path['filename'])) . ' <a class="pat-res-link" href="'.$file.'">'.$file_path['filename']. '.' . $file_path['extension'].'</a></h3>';
		if ($info_string): echo '<div class="pat-res-info">' . $info_string . '</div>'; endif;
		echo '</header>';
	}
	else {
		echo '<div class="searchable pat-res-space" title="' . ucwords(str_replace('-',' ',$file_path['filename'])) . '" id="' . $file_id . '"></div>';
		echo '<header class="pat-res-sub-header pat-res"><h4 class="pat-res-pattern-heading">' . ucwords(str_replace('-',' ',$file_path['filename'])) . ' <a class="pat-res-link" href="'.$file.'">'.$file_path['filename']. '.' . $file_path['extension'].'</a></h4>';
		if ($info_string): echo '<div class="pat-res-info">' . $info_string . '</div>'; endif;
		echo '</header>';
	}
  ...
  echo $html_string;
  echo '</div>';
  echo '<div class="pat-res-source pat-res">';
  echo '<h5 class="pat-res-source-heading">HTML</h5>';
  echo '<pre class="prettyprint lang-html">';
  echo $html_string_code;
  echo '</pre>';
  if (file_exists($css_file) || $css_string) {
    echo '<h5 class="pat-res-source-heading">CSS</h5>';
